...
class Facebook_page extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			...
			esc_html__( 'A Facebook Page', 'fletwp' ), // Name
			array( 'description' => esc_html__( 'Display Your FaceBook Page Feed Widget', 'fletwp' ), ) // Args
		);
	}
	...
	public function form( $instance ) {
		$title = ! empty( $instance['title'] ) ? $instance['title'] : esc_html__( 'Your Title... ', 'fletwp' );
		$pagelink = ! empty( $instance['pagelink'] ) ? $instance['pagelink'] : esc_html__( 'envato', 'fletwp' );
		$hidecover = ! empty( $instance['hidecover'] ) ? $instance['hidecover'] : esc_html__( 'false', 'fletwp' );	
		$width = ! empty( $instance['width'] ) ? $instance['width'] : esc_html__( '360', 'fletwp' );	
		$height = ! empty( $instance['height'] ) ? $instance['height'] : esc_html__( '500', 'fletwp' );	
		...
		<!-- Title Of Widget  -->
			<p  style="margin-bottom:20px;">
				<label style=" text-transform: capitalize;font-weight: 800; margin-bottom:20px;" 
					for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_attr_e( 'Title:', 'fletwp' ); ?>
				</label> 
				<input class="widefat" 
		...
		<!-- The Url Of FaceBook Wideget -->
			<p style="margin-bottom:20px;">
				<label 
					...
					for="<?php echo esc_attr( $this->get_field_id( 'pagelink' ) ); ?>"><?php esc_attr_e( 'Your Facebook Page Unique Name', 'fletwp' ); ?>
		...
		<!-- Hide Cover For FaceBook Widget-->
			<p style="margin-bottom:20px;">
				<label 
					style=" text-transform: capitalize;font-weight: 800;"
					for="<?php echo esc_attr( $this->get_field_id( 'hidecover' ) ); ?>"><?php esc_attr_e( 'Facebook Page Cover Display or Hide Option:', 'fletwp' ); ?>
		...
		<!-- width Of FaceBook Page -->
			<p  style="margin-bottom:20px;">
					...
					<label style=" text-transform: capitalize;font-weight: 800; margin-bottom:20px;" 
						for="<?php echo esc_attr( $this->get_field_id( 'width' ) ); ?>"><?php esc_attr_e( 'width:', 'fletwp' ); ?>
					</label> <br>
					<span style="
						font-size:12px 
						margin-bottom: 10px;
		...
		<!-- Height Of FaceBook Page -->	
			<p  style="margin-bottom:20px;">
					...
					<label style=" text-transform: capitalize;font-weight: 800; margin-bottom:20px;" 
						for="<?php echo esc_attr( $this->get_field_id( 'height' ) ); ?>"><?php esc_attr_e( 'height:', 'fletwp' ); ?>
					</label> <br>
					<span style="
						font-size:12px 
						margin-bottom: 10px;
